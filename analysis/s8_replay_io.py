...
def replay_paths(submission: str) -> list[Path]:
    ...
    Episode id order == createTime order — verified monotonic on the 178-episode
    55586926 CSV 2026-08-23 (`id-order == createTime-order` True). So id-ascending is
    createTime-ascending, and Task 1's "first 75" needs no episodes listing / download.
    """
    d = SUBMISSIONS[submission]
    files = glob.glob(str(d / "episode-*-replay.json"))
    return sorted((Path(p) for p in files), key=lambda p: _eid_from_name(p))
