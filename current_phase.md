...
~~`v1h.2d` κλείσιμο & submission~~ ✅ → ~~`L1` ladder diagnostic~~ ✅ →
~~`v1j` scale-out `{24,12}`~~ ⛔ STOP στο smoke → **επόμενο: hire-path ή ενδιάμεσο tile count** ⬅
**ΤΩΡΑ** → **`Β.2` meta-bench** → **`v1i` sell-ahead** → BBO / Φάση 3.
...
**Υπόθεση (διαψευσμένη στο smoke ως `{24,12}`):** έχουμε ~19 πληρωμένα αλλά αδρανή tiles και το
sell-side headroom που έλειπε (§3). Το wheat είναι το **μόνο** προϊόν χωρίς cliff…

~~Εκτέλεση — φθηνά controls~~ ✅ ολοκληρώθηκαν· διαγώνιο αρνητικό ⇒ STOP πριν από DEV/holdout.

**Ρητά εκτός scope του v1j:** αγορά **SE** · αλλαγή σύνθεσης κοπαδιού · αλλαγή strawberry.
...
| **v1j follow-up** (hire-path για 11-12 hands **ή** ενδιάμεσο tile count με 10 hands) | 08-10 → 08-18 | ⬅ **ΤΩΡΑ** — πριν ξανανοίξει scale-out |
