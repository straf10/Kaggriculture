...
> `engine_reference/kaggriculture.py` (pinned στο **`kaggle-environments==1.32.7`**).
>
> **Πριν γράψεις οποιαδήποτε λογική agent, αυτό το αρχείο διαβάζεται πρώτο.** Το
> [ROADMAP.md](ROADMAP.md) §3.1 δίνει τη στρατηγική περίληψη· εδώ είναι μόνο τα
> σημεία όπου η αφελής ανάγνωση των docs παράγει **λάθος agent**.
>
> Τελευταία ενημέρωση: **2026-08-15** (καταγραφή του bump 1.32.6→1.32.7, D28 — το `hinge`
> curve σε CARROT/TOMATO/EGG· επαλήθευση εγκατεστημένης έκδοσης, byte-ισότητας
> `engine_reference/` και `pytest tests/` 268 passed. Προηγούμενη: 2026-08-11, bump
> 1.32.5→1.32.6, D27).
...
| D27 | **[1.32.6 — η ανακοινωμένη balance change, ΖΩΝΤΑΝΗ]** Τρεις αλλαγές, όλες επαληθευμένες **στο εγκατεστημένο πακέτο** (2026-08-11): **(α)** το `TOWN_CENTER_DEMAND_SCHEDULE` (`[(20,4),(10,2),(0,1)]`) **διαγράφηκε** — το `_town_consume` κάνει πλέον flat `-= 1` ανά προϊόν· **(β)** `townCenterSellInterval` json default **12 → 24** (1 tick/μέρα αντί 2)· **(γ)** το shop unlock έχασε το `remaining = [s for s in SHOPS if s not in unlocked]` φίλτρο — τα shops κληρώνονται **με επανάθεση**, με νέα σταθερά `MAX_SHOP_INSTANCES = 8`, και **κάθε instance καταναλώνει ανεξάρτητα**. Αριθμητικά: town centre **140 → 30 μονάδες/προϊόν/σεζόν (−79%)**, με −87,5% ακριβώς στις μέρες 20-29· `E[διακριτοί τύποι shop] = 8·(1−(7/8)⁸) = 5,25`· `P(τύπος απών) = (7/8)⁸ = 34,4%`· `P(και οι 8 παρόντες) = 8!/8⁸ = 0,24%`. **Καμία άλλη αλλαγή** έναντι 1.32.5: `MARKET_PARAMS`, τιμές, cliffs, `_spawn_weeds`, `TOWN_CENTER_PRODUCTS` byte-identical. ⚠️ Πρακτικές συνέπειες: το **MELON** (0/8 shops) εξαρτάται 100% από ένα town centre που έχασε το 79% της απορρόφησής του· το `town_pin='schedule'` του harness δειγματίζει πλέον κατανομή που εμφανίζεται στο **0,24%** των επεισοδίων (χρησιμοποίησε `basket`)· και το «κράτα και πούλα αργά» **έπαψε να ισχύει** — η καμπύλη ζήτησης είναι επίπεδη | ✅ TESTED (`test_engine_reference_matches_installed` byte tripwire + `tests/test_engine_facts.py:535` `townCenterSellInterval == 24` + `tests/test_agent_guards.py:124` για την απουσία του `TOWN_CENTER_DEMAND_SCHEDULE`· `pytest tests/` **226 passed** στις 2026-08-11) | — |
| D28 | **[1.32.7 — η δεύτερη balance change· ΕΓΚΑΤΕΣΤΗΜΕΝΗ, ΟΧΙ ΑΚΟΜΑ ΖΩΝΤΑΝΗ]** Αλλαγή **μόνο αγοράς**, μόνο στη **below-I0** (scarcity) πλευρά, μόνο σε **CARROT/TOMATO/EGG** — τα τρία προϊόντα με μηδενική παραγωγή στο high-level play. Νέο shape `hinge` στο `_shape`: με `u = x/T`, `f(x) = u + 8·max(0, u−1)²`, με `HINGE_GAIN = 8.0`. Είναι το **μόνο** shape που κλιμακώνεται με το `T` (το `_shape` πήρε τρίτο προαιρετικό όρισμα· degenerates σε `linear` όταν λείπει το `T`), και `f(T) == 1` εξ ορισμού ώστε το `target` να κρατάει την ίδια σημασία. Νέες παράμετροι: `CARROT` below `log`/0,20 → **`hinge`/1,00**· `TOMATO` below `linear`/0,40 → **`hinge`/0,40**· `EGG` below `linear`/0,40 → **`hinge`/0,40`**. ⚠️ **Δύο ασυμμετρίες που το ίδιο το PR υποβαθμίζει:** (α) **TOMATO/EGG είναι αυστηρό no-op** από depletion 0 έως T (το amp του `linear` κανονικοποιεί σε `x/T`, που είναι ακριβώς ο κάτω-από-το-γόνατο κλάδος του `hinge`) — αλλάζει **μόνο** η ουρά πέρα από το γόνατο· (β) το **CARROT αλλάζει από depletion 1 και πάνω** (433 από 451 τιμές στο 0..T), γιατί άλλαξε **shape ΚΑΙ target**. Πίνακες (depletion → τιμή): CARROT 0/225/450/600/900/1200 → $35/$52/**$70**/$113/$385/$906 (ήταν $35/$41/**$42**/$42/$43/$43 — δηλαδή πρακτικά αδρανής)· TOMATO 250/300/390/534 → $102/$144/$280/$660 (ήταν $90/$96/$107/$124)· EGG 415/498/664 → $85/$120/$250 (ήταν $75/$80/$90). Η **glut πλευρά είναι byte-identical** και για τα τρία, και τα υπόλοιπα έξι προϊόντα (WHEAT/STRAWBERRY/MELON/MILK/WOOL/FERTILIZER) είναι **byte-identical παντού**. Καμία αλλαγή σε turn order, RNG, `_spawn_weeds`, yields, shops ή town demand. 🔎 **Rollout:** το `configuration` **δεν** μεταφέρει τα `MARKET_PARAMS`, άρα το bump **δεν** ανιχνεύεται όπως το D27 (`townCenterSellInterval`) — ανιχνεύεται μόνο **συμπεριφορικά**, από ζεύγη `market.inventory`/`market.prices` του ίδιου του replay (`analysis/v1t_engine_probe.py`). Μέτρηση 2026-08-15 στο dataset `kaggriculture-episodes-2026-08-14`: **1.32.6, ομόφωνα** — το bump δεν έχει φτάσει στα ζωντανά επεισόδια | ✅ TESTED (`tests/test_v1t_hinge.py`, 14 guards: `f(T)==1`, degeneration, πίνακες τιμών, no-op TOMATO/EGG κάτω από το γόνατο, το 433-τιμών CARROT delta, glut untouched· + διευρυμένο `test_vendored_constants_and_prices_match_pinned_engine` σε ±3T αντί για 3 σημεία· `pytest tests/` **268 passed** στις 2026-08-15) | — |
...
**κάθε line reference σε docs/ και σε σχόλια κώδικα έχει σιωπηλά μετακινηθεί** και πρέπει να
ελεγχθεί.

⚠️ Το `pip install -U` παραπάνω **δεν τρέχει ποτέ ενώ τρέχει gate** — ακυρώνει τη σύγκριση. Ο
detector χωρίς install (`pip index versions` → `pip download --no-deps --no-binary :all:` →
`diff` του `.py` **και** του `.json`) είναι στο [ROADMAP §2.1.7](ROADMAP.md).
