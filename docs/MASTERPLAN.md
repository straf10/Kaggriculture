...
>
> ### 🔴 **[2026-08-09] Η balance change ΚΥΚΛΟΦΟΡΗΣΕ: `kaggle-environments 1.32.6`**
>
> Οι οργανωτές ανακοίνωσαν στο discussion «these changes are rolling out and hitting the
> leaderboard — upgrade to >= 1.32.6». **Το επαληθεύσαμε με diff του sdist έναντι του
> [engine_reference/](../engine_reference/) (=1.32.5), χωρίς install.** Το diff περιέχει
> **ακριβώς** ό,τι είχαμε μοντελοποιήσει ως «ανακοινωμένο, αδημοσίευτο» — τίποτα παραπάνω,
> τίποτα λιγότερο:
>
> | Αλλαγή | 1.32.5 | 1.32.6 |
> |---|---|---|
> | `townCenterSellInterval` (json default) | `12` (2 ticks/μέρα) | **`24`** (1 tick/μέρα) |
> | `TOWN_CENTER_DEMAND_SCHEDULE` | `[(20,4),(10,2),(0,1)]` | **διαγράφηκε** — flat `-= 1` |
> | Shop unlock | `rng.choice(sorted(remaining))` (χωρίς επανάθεση, 8 **διακριτά**) | **`rng.choice(sorted(SHOPS))` με επανάθεση**, νέα σταθερά `MAX_SHOP_INSTANCES = 8` |
>
> Καμία άλλη αλλαγή: `MARKET_PARAMS`, τιμές, cliffs, `_spawn_weeds`, RNG coupling (§2 #6/#7),
> `TOWN_CENTER_PRODUCTS` (FERTILIZER πάντα εκτός) **byte-identical**.
>
> **Συνέπεια:** κάθε παράγραφος αυτού του εγγράφου που λέει «ανακοινωμένη / αδημοσίευτη /
> όταν έρθει» αναφέρεται πλέον στο **σημερινό** engine. Οι αριθμοί που είχαμε προϋπολογίσει
> (TC −79%· E[διακριτοί τύποι] = 5,25· **P(κανένα YARN_STORE) = 34,4%**· P(και τα 8 παρόντα) =
> 0,24%) **δεν ήταν πρόβλεψη — είναι η τρέχουσα κατάσταση**. Το ότι τους είχαμε ήδη υπολογίσει
> **και** είχαμε τρέξει το herd screen πάνω σε `basket_for` (επανάθεση) σημαίνει ότι μπαίνουμε
> στο νέο καθεστώς **προ-ελεγμένοι**, ενώ το meta μπαίνει τυφλό (φυτεύει 11,6 melon
> seeds/παίκτη σε ένα προϊόν που μόλις έχασε το 79% του μοναδικού του αγοραστή).
> Επιχειρησιακή εκτέλεση του bump: `current_phase.md` §v1h.1.
>
...
- **Town = μοτέρ ζήτησης**: shops ξεκλειδώνουν κάθε 3 μέρες (τυχαία σειρά από 8), καθένα τρώει το μενού του κάθε 4 turns (6 μον./προϊόν/μέρα, single-product ×2 = 12/μέρα)· town center κάθε 12 turns, **×2 από μέρα 10, ×4 από μέρα 20** (README.md:169-173· viz cell 34). **Κανένα shop δεν αγοράζει melon ή fertilizer** — το melon στηρίζεται μόνο στο town center → η πιο εύθραυστη τιμή του παιχνιδιού (viz cell 35).
  ...
  > 🔴 **[1.32.6 — ισχύει ΤΩΡΑ] Η προηγούμενη γραμμή περιγράφει το 1.32.5 και έχει αντικατασταθεί.**
  > Shop demand **αμετάβλητο** (6/προϊόν/μέρα ανά *instance*, single-product ×2). Town center:
  > **1 tick/μέρα, flat ×1** ⇒ **30 μονάδες/προϊόν/σεζόν αντί για 140 (−79%)**, με −87,5% ακριβώς
  > στις μέρες 20-29 (τη φάση ρευστοποίησης). Shops κληρώνονται **με επανάθεση** μέχρι 8
  > instances ⇒ **5,25 αναμενόμενοι διακριτοί τύποι**, `P(τύπος απών) = (7/8)⁸ = 34,4%`,
  > `P(και οι 8 παρόντες) = 8!/8⁸ = 0,24%`. Δύο συνέπειες που αλλάζουν στρατηγική:
  > (i) το **melon** (0/8 shops, 100% TC-εξαρτημένο) παύει να είναι παίξιμο crop — §3.2#5·
  > (ii) το **WOOL** (YARN_STORE μόνο) μένει χωρίς αγοραστή σε **1 στα 3** επεισόδια, κόστος
  > μετρημένο **$12.077/ep** και **καμία** sell-side αντιστάθμιση (`current_phase.md` §v1g.2 γ).
...
> 🔴 **[ενημ. 2026-08-09 — τρίτη ανάγνωση, από field actions αντί για end-state]** Δύο νέα
> replay-based notebooks (§3.2ter) μετρούν την ώρια φάρμα **από τις ενέργειες του replay**, όχι
> από τα επιζώντα tiles — δηλαδή **παρακάμπτουν το artifact του ⚠️(ii) παραπάνω**. Η κοινή
> διαδρομή των ranks 3-20 είναι **~8 cow / 6 sheep · 23 strawberry · 31 wheat · 3 quadrants ·
> 12 hands**. Τρία σχόλια: **(α)** επιβεβαιώνει την strawberry-βαριά ανάγνωσή μας (23 ≈ τα δικά
> μας 24) — το «6 strawberry» ήταν όντως κάτω φράγμα· **(β)** δείχνει **31 wheat tiles** έναντι
> των **12** δικών μας: το πιο συγκεκριμένο, μετρήσιμο κενό που έχουμε σήμερα, και σε ένα προϊόν
> που μετά το 1.32.6 είναι το **μόνο** χωρίς cliff και με 5/8 shop κάλυψη· **(γ)** το «12 hands»
> επανεμφανίζεται και **ανοίγει ξανά** το ερώτημα του crew — οι δύο μετρήσεις (5-6 vs 12) μετρούν
> διαφορετικά πράγματα (hands *παρόντα* vs hires *εκτελεσμένα*), και η 08-09 είναι η μεγαλύτερη
> (144 επεισόδια / 40 ομάδες). Δεν αλλάζουμε `hands_target` χωρίς gate — αλλά το v1h′ έδειξε ήδη
> ότι **crew και tiles πληρώνουν μόνο μαζί**, άρα το (β) και το (γ) είναι **ένα** increment.
...
5. ⚠️ **[ενημ. 2026-08-07: υποβαθμίζεται σε «μη παίξιμο», βλ. τέλος του σημείου]** **Melon = χρυσός με ημερομηνία λήξης.** $142/tile-day σε base — αλλά τετραγωνική κατάρρευση (158 μονάδες → $1) και μηδενική στήριξη από shops. Παίζεται *οπορτουνιστικά*: λίγα tiles, πώληση σε τρίκλες όταν price ≥ threshold, και **παρακολούθηση της φάρμας του αντιπάλου** — αν εκείνος φυτεύει melons μαζικά, ωρίμανσή τους σε ~10 μέρες σημαίνει επερχόμενο glut: πούλα πριν από αυτόν ή απόφυγε το crop. Παρότι είναι το δεύτερο πιο συχνό πρωτεύον crop στο ladder (n=1380), win rate μόλις ~49% — πολλοί αντίπαλοι το παίζουν ως κύριο crop, όχι οπορτουνιστικά, και προφανώς δεν βγαίνουν κερδισμένοι σε σχέση με το Strawberry.
   ...
   > **[ενημ. 2026-08-07 · ✅ ΕΠΙΒΕΒΑΙΩΘΗΚΕ 2026-08-09 — η αλλαγή είναι ΖΩΝΤΑΝΗ στο 1.32.6]
   > Το melon παύει να είναι στρατηγική επιλογή.** **Κανένα** από τα 8 shops δεν αγοράζει melon
   > (ούτε fertilizer) — είναι **100%
...
6. **Timing πωλήσεων ↔ town ramp**: η ζήτηση ×2 από μέρα 10, ×4 από μέρα 20 και τα shops προστίθενται κάθε 3 μέρες — **ίδια αγαθά αξίζουν περισσότερα αργότερα**, εντός των ορίων του shed cap 100. Trickle selling πάντα (orders λύνονται unit-by-unit)· ποτέ dump στο floor.
   ...
   > 🔴 **[ενημ. 2026-08-09] Η ημερομηνία λήξης ήρθε: το «αξίζουν περισσότερα αργότερα» ΕΠΑΨΕ ΝΑ
   > ΙΣΧΥΕΙ.** Το 1.32.6 καταργεί ολόκληρο το ramp (flat ×1) **και** πάει το
   > `townCenterSellInterval` 12 → 24 (1 tick/μέρα αντί 2). Σεζόν ανά προϊόν:
   > `10×2 + 10×4 + 10×8 = 140` → `30×1 = 30` μονάδες, δηλαδή **−79%**, με **−87,5%** στις μέρες
   > 20-29 — ακριβώς τη φάση ρευστοποίησης. *(Η προηγούμενη διατύπωση «ανακοινωμένες,
   > αδημοσίευτες» ίσχυε για το 1.32.5 και είναι πλέον ιστορική.)*
   > **Αυτό το σημείο #6 ανατρέπεται πλήρως: η καμπύλη ζήτησης είναι πλέον επίπεδη, άρα το
   > κίνητρο δεν είναι «κράτα και πούλα αργά» αλλά «πούλα σταθερά, νωρίς, και πριν τον
   > αντίπαλο».** Το ίδιο συμπέρασμα που είχε ήδη βγάλει το §v1g.2(δ) για το fertilizer
   > γενικεύεται τώρα σε **κάθε** προϊόν. Το trickle selling παραμένει σωστό και στα δύο
   > καθεστώτα — αυτό που άλλαξε είναι ότι το **αργό** trickle δεν αποζημιώνεται πλέον.
   > Πλήρης ανάλυση: [meta/competition_updates.md 2026-08-07](meta/competition_updates.md)·
   > εκτέλεση: `current_phase.md` §v1h.1 / §v1i.
...
8. **Παρατήρηση `unlocked_shops`**: ποια shops άνοιξαν είναι τυχαίο ανά episode — το crop mix mid-season πρέπει να προσαρμόζεται στη ζήτηση που όντως ξεκλείδωσε (π.χ. Pet Café → 12 carrots/μέρα, Yarn Store → 2× wool).
   ...
   > στην καλύτερη δυνατή ρύθμιση, 0/8 episode wins σε κάθε πόλη).
   >
   > > 🔴 **[ενημ. 2026-08-09, μετά το v1h.1] Η ΑΙΤΙΟΛΟΓΗΣΗ παρακάτω έπαψε να ισχύει στο 1.32.6·
   > > το ΣΥΜΠΕΡΑΣΜΑ («το sell layer δεν αντισταθμίζει») όχι.** Το «production-constrained,
   > > ποτέ glut-constrained» μετρήθηκε σε engine που απορροφούσε **140** μονάδες/προϊόν/σεζόν
   > > από το town centre. Σήμερα απορροφά **30**. Μετρημένο στο 1.32.6 (`gate_v1h1_devscreen`
   > > + single episodes): **είμαστε glut-constrained στο MILK** — μέση τιμή πώλησης **$31-37**
   > > σε περίπου μισά τα seeds (base $160), **2.370 μονάδες στα ≤$5** σε 48 seeds, και
   > > `shed_overflow_burnt` έως 230/seed. Το WOOL μένει υγιές ($197-243), άρα ο «σπάνιος
   > > αγοραστής» **δεν** ήταν ποτέ το κύριο ρίσκο — ο **κορεσμός σε mirror** ήταν, ακριβώς
   > > όπως το είχε δείξει το herd screen της 08-08 και όπως δεν το είχαμε γενικεύσει.
   > > **Ο μοχλός παραμένει παραγωγικός, όχι sell-side** (το floor εξακολουθεί να μην ανακτά
   > > τίποτα) — αλλά δείχνει πλέον προς **λιγότερες αγελάδες**: `current_phase.md` §v1h.2 (γ).
   >
   > Αιτία (όπως ίσχυε στο 1.32.5): ο agent είναι
...
### 3.2ter Η ladder έγινε **μονοκαλλιέργεια** (2026-08-09) — το πιο σημαντικό στρατηγικό γεγονός μετά το 1.32.6

> **Πηγές (και οι δύο EVIDENCE, όχι πηγή κώδικα — Ανοιχτό #11):**
> [notebooks/two-private-bots-beating-kaggriculture-meta.ipynb](../notebooks/two-private-bots-beating-kaggriculture-meta.ipynb)
> (δομημένη εξαγωγή openings από 15 replays, 3 ανά top-5 ομάδα) και
> [notebooks/kaggriculture-findings-from-zero-to-top-meta.ipynb](../notebooks/kaggriculture-findings-from-zero-to-top-meta.ipynb)
> (replay-hunting diary c14→c68· στο 09-08 κατέβασε **10 replays ανά ομάδα από τις live top-20
> = 200 επιλογές**). ⚠️ Το δεύτερο **ενσωματώνει εκτελέσιμο base64+zlib blob** του agent του —
> **δεν εκτελείται**, ούτε αποσυμπιέζεται· διαβάζουμε μόνο τα markdown/πίνακες.

**Το εύρημα σε μία γραμμή: ~75% της ladder παίζει την *ίδια* πολιτική.**

| Μέτρηση (2026-08-09) | Τιμή |
|---|---|
| Ένα **μοναδικό field hash** εμφανίστηκε σε | **144 επεισόδια από 40 ομάδες** |
| Ranks 3-20 ταυτόσημα σε field actions | **99-100%** |
| Top-5 ομάδες με **byte-identical opening** (fork του δημόσιου `kaitofukami v23`) | **3 από 5** |
| ELO ταβάνι αυτού του cluster | **3.117 – 3.131** |

**Οι τρεις clusters στην κορυφή:**

| Cluster | Ομάδες | ELO | Day-0 υπογραφή | Ώριμη φάρμα |
|---|---|---|---|---|
| `v23_fork` (δημόσιο meta) | 3 από top-5, ~40 ομάδες συνολικά | 3.117-3.131 | 5 hires · 2 cow + 2 sheep · 7 wheat / **12 melon** seeds · 0 strawberry | ~8c/6s, **23 strawberry, 31 wheat**, 3 quadrants, 12 hands |
| `sheep_first_hybrid` (HealthStone, #2) | 1 | **3.132,9** | **3 hires · 1 cow + 4 sheep** · 5 wheat / 5 melon | 8c/**4s**, 3 quadrants, 12 ζώα |
| `counter_meta` (Seb, **#1**) | 1 | **3.201,1** | **14 hires** · 3 cow + 2 sheep · 14 wheat / 3 melon | 9c/**11s**, **4 quadrants (SE συμπεριλαμβανομένου)**, **20 ζώα** |

**Πέντε συνέπειες, με φθίνουσα βεβαιότητα:**

1. **Το bench μας μετρά λάθος αντίπαλο — αυτό είναι το μεγαλύτερο ανοιχτό κενό.** Κάθε gate μας
   είναι `v_n` εναντίον `v_{n-1}`, δηλαδή mirror. Στην πραγματική ladder ο αντίπαλος είναι, σε
   3 στα 4 παιχνίδια, **μία συγκεκριμένη ξένη πολιτική**. Ένα mirror-only gate βελτιστοποιεί
   την ικανότητά μας να νικάμε **τον εαυτό μας**. Αντίμετρο **που δεν παραβιάζει το Ανοιχτό
   #11**: clean-room bench opponent χτισμένος από τα **δημοσιευμένα στατιστικά** της κοινής
   διαδρομής (8c/6s · 23 strawberry · 31 wheat · 3 quadrants · 12 hands · ημερολόγιο πώλησης
   wheat d2 / milk d10 / melon d10 / strawberry d18 / wool d9 / fertilizer d3), **ποτέ** από
   trajectories ή από κώδικα competitor notebook. Στατιστικά ⇒ επιτρεπτά· tapes/κώδικας ⇒ όχι.
   Εκτέλεση: `current_phase.md` §Β.2.
2. **Το v1i (sell-ahead) γίνεται το υψηλότερης αξίας εναπομείναν item, με τρίτη ανεξάρτητη
   επιβεβαίωση.** Σε μονοκαλλιέργεια το κύμα πώλησης του αντιπάλου είναι **προβλέψιμο για τα
   ¾ της ladder**. Τρεις ανεξάρτητες μετρήσεις τρίτων λένε το ίδιο: `Hamburger Clone Quad H1`
   **6-0, μέσο margin +1.865,7** μόνο από **one-turn front-run**· το `c15` πήρε **14-2 εναντίον
   της ίδιας του της base tape** προσθέτοντας μόνο τον front-run· και το `c18` άλλαξε **20
   field turns αλλά 112 market turns** έναντι του c16 και κέρδισε **35-5** — «*the current edge
   comes from inventory-sale timing rather than another change in herd composition*». Αυτό
   ταυτίζεται με το δικό μας §3.3 και με το [agents-1](meta/ladder_snapshots.md#agents-1)
   (31-1, +$2.304).
3. **Ο μηχανισμός του `c68` είναι υλοποιήσιμος από εμάς σήμερα, και μας λείπει μόνο το εύκολο
   μισό.** Περιγράφεται ως: παρατήρησε τη μεταβολή του market inventory ανά premium προϊόν →
   **αφαίρεσε τη δική σου πώληση και το ντετερμινιστικό town drain** → προσάρμοσε το υπόλοιπο
   σε horizon 1-6 → τρέξε ένα turn μπροστά. Ο δύσκολος όρος («ντετερμινιστικό town drain») είναι
   **ακριβώς** το [agent/demand.py](../agent/demand.py) `npc_daily_demand`, που ήδη έχουμε
   engine-pinned από το v1g.2(β) — γραμμένο για άλλο σκοπό, που διαψεύστηκε. **Το εργαλείο
   επιβιώνει της υπόθεσης που το γέννησε.**
4. **Το «SE ποτέ» είναι πλέον λάθος ως απόλυτο.** Ο **rank-1** (Seb) παίζει **4 quadrants και 20
   ζώα**. Σωστή διατύπωση: *το modal route δεν αγοράζει SE· η μία ομάδα που κερδίζει το modal
   route το αγοράζει.* Δεν αρκεί για να το αγοράσουμε (n=1, και τα per-game coins του Seb είναι
   **χαμηλότερα** — ~$57k — κερδίζει σε W/L, όχι σε $), αλλά αρκεί για να πάψει το SE να είναι
   κλειστό ερώτημα. Το ίδιο και τα **hands**: 12 στο modal route, **14 hires day-0** στον #1,
   έναντι του δικού μας 6→10. Η fib καμπύλη λέει 10 hands = **$143/μέρα**, 12 = **$376**,
   14 = **$986** — το «φθηνό» τελειώνει στα ~10-11.
5. **Το «διαφοροποίησε το 2ο slot» γίνεται κανόνας, όχι σύσταση.** Ο κατάλογος κοινών λαθών του
   findings notebook έχει ρητά: *«two near-identical active submits → meta shift kills both»*.
   Κρατάμε **v1g + v1h**, που διαφέρουν μόνο κατά το SW/wheat — και **μόλις έγινε meta shift**
   (1.32.6). Βλ. `current_phase.md` §Α.3.

> **Το «διάβασμα» του ELO ταβανιού, με προσοχή.** Το ότι ο `v23_fork` κολλάει στα 3.117-3.131
> ενώ δύο ιδιωτικοί bots κάθονται από πάνω **δεν** σημαίνει ότι οι ιδιωτικοί είναι «καλύτεροι
> agents» με απόλυτους όρους. Σε πληθυσμό όπου το 75% είναι κλώνοι, ένα bot αξιολογείται σχεδόν
> αποκλειστικά από το **πόσο καλά κερδίζει τον κλώνο**. Αυτό είναι *ακριβώς* το είδος edge που
> ένα mirror-only tuning loop δεν μπορεί να βρει, και *ακριβώς* το είδος edge που εξαφανίζεται
> αν το meta αλλάξει (και άλλαξε: το `kaitofukami v25 «Meta Reset»` μετακινήθηκε σε
> **sheep-first basin** αναφέροντας **41/41 wins** έναντι του v23-fork cluster). Άρα:
> **κυνηγάμε το anti-clone edge, αλλά δεν το αφήνουμε να γίνει η μόνη μας πηγή νίκης** — το
> βραβείο είναι top-10 σε Bradley-Terry μετά το deadline (§1), όχι rank-1 σε ένα στιγμιότυπο.

**Δύο engine facts από αυτά τα notebooks, ελεγμένα ανεξάρτητα στο δικό μας `engine_reference/`:**

- **Το step 718 είναι το τελευταίο εκτελέσιμο turn· το action index 719 δεν εκτελείται ποτέ.**
  Επαληθεύτηκε: `interpreter` θέτει `status = "DONE"` όταν `step >= episodeSteps - 2`
  (`engine_reference/kaggriculture.py:946-949`), δηλαδή στο step 718 — μετά από αυτό δεν
  ζητούνται actions. Ένα SELL **στο** 718 εκτελείται κανονικά (`_process_market` τρέχει πριν το
  DONE). Πρακτικά για εμάς: το `liquidation_day = 26` αφήνει τεράστιο περιθώριο, άρα δεν είναι
  bug — αλλά **κάθε** endgame σχέδιο πρέπει να στοχεύει το 718, όχι το 719.
- **`SELL` βλέπει μόνο το shed· το `HARVEST` γεμίζει το unit inventory.** Ήδη γνωστό μας (v1g.1
  shed routing), αλλά αξίζει η διατύπωση των άλλων: *«if you harvest 90 melons and never DROP
  while shed-adjacent, the market may not see them until end-of-day auto-drop — often too late
  to fund same-turn land/animal buys»*. Είναι το πιο συχνά αναφερόμενο λάθος του meta.

### 3.2quater Το «profit per action» discussion (2026-08-09) — τι κρατάμε και τι έχει ήδη λήξει

Πηγή: [notebooks/kaggriculture-what-a-turn-is-worth.ipynb](../notebooks/kaggriculture-what-a-turn-is-worth.ipynb).
Κατατάσσει κάθε crop/ζώο σε **$ ανά farmer action** (720 actions/σεζόν, market orders δωρεάν):

`Melon 142 · Sheep+CARE 100 · Cow+CARE 79 · Cow feed-only 35 · Sheep feed-only 32 · Goose+CARE 28 · Strawberry 27 · Goose 18 · Tomato 17 · Carrot 17 · Wheat 15`

**Τι επιβεβαιώνει (και είναι χρήσιμο ως ανεξάρτητη επαλήθευση):**
- **Το CARE δεν είναι προαιρετικό.** Sheep 32 → **100 $/action** μόνο από την προσθήκη CARE, με
  τον ίδιο μηχανισμό που περιγράφει το §1 («banked bonus πληρώνεται ολόκληρο στην επόμενη
  παραγωγή ⇒ `1 + interval` μονάδες»). Ταυτίζεται με το §3.2#1 και με το v1g screen.
- **Το ίδιο το πλαίσιο «actions, όχι γη/χρήμα»** είναι το §3.1 μας, φτασμένο ανεξάρτητα.
- Το μοντέλο ποτίσματος του notebook **αναπαράγει** τα δύο yields που δηλώνει το README
  (wheat 4, carrot 3) και βγάζει melon cap 6 στην **ημέρα 10** — συμφωνεί με τον πίνακά μας §1.

**Τι είναι ήδη άκυρο ή επικίνδυνο — και γιατί αυτό είναι *καλά νέα* για εμάς:**
1. **Το #1 του πίνακα (melon) είναι το χειρότερο crop του παιχνιδιού από σήμερα.** Ο ίδιος ο
   συγγραφέας το δηλώνει ως «base prices, upper bound, priority order not revenue forecast» —
   αλλά μετά το 1.32.6 το melon είναι **0/8 shops + TC στο −79% + cliff στις 158 μονάδες**.
   Το «9,5× wheat» αντιστρέφεται. **Το discussion αυτό θα διαβαστεί από πολλούς και θα στείλει
   κόσμο σε melon** — και το `v23_fork` opening φυτεύει ήδη **12 melon seeds day-0** (§3.2ter).
   Εμείς φυτεύουμε **0**. Δεν αλλάζουμε τίποτα· απλώς επιβεβαιώνεται ότι το §3.2#5 ήταν σωστό.
2. **Το $/action αγνοάει το commute — δηλαδή αγνοάει το πραγματικό bottleneck μας.** Ο πίνακας
   μετρά ένα tile σε απομόνωση. Το δικό μας μετρημένο εμπόδιο σε **κάθε** increment (v1c ×3
   STOP, v1h′ FEED priority −1, 12-14 ζώα με 660-885 escapes) ήταν **routing/logistics**, όχι
   αξία ανά action. Το §3.1 το λέει ήδη με δεδομένα: `tiles_planted` νικητών/ηττημένων = **1,02×**.
   Άρα: **χρήσιμο ως tie-breaker κατά τη σχεδίαση, άχρηστο ως αντικειμενική συνάρτηση.**
3. **Το ίδιο το notebook παραδέχεται το κενό που μας ενδιαφέρει**: *«this tells you what to do
   with turn 400, not what to do with turn 1»*. Το opening είναι cash-flow πρόβλημα — και εκεί
   ακριβώς διαφέρουν οι τρεις top clusters (§3.2ter: $14 vs $500+ ταμείο τις μέρες 1-5).

**Το ένα πράγμα που αξίζει να υιοθετηθεί:** η **action-cost λογιστική του κοπαδιού**. Ένα cared
sheep κοστίζει **~71 unit-actions/σεζόν**, ένα cared cow **~74** (BUILD+PLACE + FEED + CARE +
HARVEST). Με 10 ζώα αυτό είναι **~730 unit-actions**, δηλαδή ένα σταθερό, μη συμπιέσιμο φορτίο
πριν καν ποτιστεί ένα tile. Αυτός είναι ο **μηχανισμός** πίσω από ένα αποτέλεσμα που είχαμε ήδη
μετρήσει αλλά όχι εξηγήσει (§3.4 σημείο 3: 12-14 ζώα ⇒ hard-gate αποτυχία σε escapes). Αξίζει να
γίνει **ρητός προϋπολογισμός** στο `_capacity_limited_targets` πριν από κάθε μελλοντικό
crew/herd sweep, ώστε το BBO να μη σπαταλά αξιολογήσεις σε συνδυασμούς που είναι εκ των προτέρων
αδύνατοι.
