...
> **[ενημ. 2026-08-07 — δύο διορθώσεις στην παραπάνω ανάγνωση]** ([meta0807](meta/ladder_snapshots.md#meta0807)):
> **(i) Το απόλυτο bank έπαψε να είναι στόχος.** Το money median της ελίτ έπεσε $125.877 → **$115.664**
> στις 08-06 **ενώ** το score-median ανέβηκε **+135 Elo**. Το bank είναι δείκτης **κορεσμού της κοινής
> αγοράς** (περισσότεροι παίκτες στην ίδια κλίμακα ⇒ χαμηλότερες τιμές για όλους), όχι δείκτης
> ποιότητας — και το BT μετρά W/L, όχι $. Κάθε στόχος τύπου «φτάσε τα $X» αντικαθίσταται από
> σχετικό κριτήριο (κερδίζω τον προηγούμενο checkpoint σε holdout· κερδίζω σε mirror margin).
> **(ii) Το modal top farm είναι measurement artifact.** Το «8 cow + 6 sheep, **καθόλου crops**»
> μετριέται στο **τέλος** του επεισοδίου, και στο engine **κανένα crop δεν επιβιώνει** ως τη μέρα 30:
> τα one-shot αδειάζουν το tile στο HARVEST (`:411-412`), και το STRAWBERRY γίνεται **WEED** λίγο μετά
> το max_yield της ηλικίας 16 (`_decay_plants`, `:742-744`) — δηλαδή ~μέρα 17-18 για φύτευση μέρας 0.
> Το ίδιο report δείχνει **1.366/1.366 seats** να πουλάνε wheat/melon/strawberry/fertilizer.
> Σωστή ανάγνωση: η ελίτ είναι **wheat+melon-βαριά νωρίς ΚΑΙ ζώα**· τα crop counts του
> [topfarms-19](meta/ladder_snapshots.md#topfarms-19) είναι **κάτω φράγμα**, όχι mix. Επίσης το
> crew της ελίτ είναι **5-6 hands** (fib cost ceiling), όχι 12 — σύμπτωση με το δικό μας μετρημένο
> `hands_target=6`.
...
7. **[νέο 2026-08-07] Το ίδιο stream κληρώνει και το shop unlock — δηλαδή το seed ΔΕΝ σταθεροποιεί την πόλη.** Επέκταση του #6, με σοβαρότερες συνέπειες για τη μεθοδολογία απ' ό,τι το weed σκέλος. Στο `_end_of_day` (`engine_reference/kaggriculture.py:847-877`) το **ίδιο** per-day `rng` που μόλις κατανάλωσαν τα `_spawn_weeds` και των δύο παικτών χρησιμοποιείται **αμέσως μετά** για το `rng.choice(sorted(remaining))` του shop unlock. Άρα η θέση της κλήρωσης στο stream ισούται με `(άδεια unlocked tiles player 0) + (άδεια unlocked tiles player 1)` εκείνο το βράδυ. Τρεις μετρημένες συνέπειες (ανεξάρτητη επαλήθευση σε 1.32.5, community notebook `notebooks/your-seed-does-not-fix-the-town.ipynb`, **evidence όχι πηγή κώδικα** — Ανοιχτό #11):
   - **Ένα tile διαφορά, σε οποιαδήποτε από τις δύο φάρμες, ξαναρίχνει ΟΛΗ την επόμενη ακολουθία shop unlocks** — και του αντιπάλου. Επιβεβαιωμένο σε πραγματικό episode με ταυτόσημα και τα 720 actions του player 0, σε 12/12 seeds. Σε καθαρούς όρους RNG, μία μονάδα μετατόπιση αλλάζει το κληρωμένο shop στο **65%** των seeds (ένα πλήρως ανεξάρτητο re-roll θα έδινε 87,5%).
   - **Η πόλη αξίζει χρήματα, και η έκθεση κλιμακώνεται με τον όγκο εμπορίου.** Σταθερό seed, σταθερός αντίπαλος, μόνο η σειρά των 8 shops μεταβάλλεται: ο ελάχιστα-εμπορικός `starter` κινείται **<1%**, ένας crop-farm agent **~14%** (worst→best draw). Μετά τη balance change (§7 ρίσκο #9) το ίδιο μέγεθος πάει σε **36%**.
   - **Μεθοδολογικός κανόνας** (πλήρης διατύπωση: §6 «Πρωτόκολλο μέτρησης», επιχειρησιακή μορφή: `current_phase.md` §Πρωτόκολλο): knob που μπορεί να αλλάξει την **occupancy** (labour/crew, φύτευση, συγκομιδή, DIG, BUY_LAND, τοποθέτηση ζώων) **δεν** συγκρίνεται έγκυρα σε σταθερό seed· knob που αλλάζει **μόνο** σειρά/ρυθμό market orders συγκρίνεται.

   **Δεν είναι exploit.** Το episode seed αφαιρείται από το configuration πριν το δει οποιοσδήποτε agent (`resolve_episode_seed`), οπότε ένας agent μπορεί να **μετατοπίσει** την κλήρωση αφήνοντας ένα tile άδειο, αλλά δεν έχει τρόπο να ξέρει προς τα πού. Το κόστος είναι **εγκυρότητα μέτρησης**, όχι δικαιοσύνη — πρόβλημα του tuning loop, όχι της ladder.
...
5. ⚠️ **[ενημ. 2026-08-07: υποβαθμίζεται σε «μη παίξιμο», βλ. τέλος του σημείου]** **Melon = χρυσός με ημερομηνία λήξης.** $142/tile-day σε base — αλλά τετραγωνική κατάρρευση (158 μονάδες → $1) και μηδενική στήριξη από shops. Παίζεται *οπορτουνιστικά*: λίγα tiles, πώληση σε τρίκλες όταν price ≥ threshold, και **παρακολούθηση της φάρμας του αντιπάλου** — αν εκείνος φυτεύει melons μαζικά, ωρίμανσή τους σε ~10 μέρες σημαίνει επερχόμενο glut: πούλα πριν από αυτόν ή απόφυγε το crop. Παρότι είναι το δεύτερο πιο συχνό πρωτεύον crop στο ladder (n=1380), win rate μόλις ~49% — πολλοί αντίπαλοι το παίζουν ως κύριο crop, όχι οπορτουνιστικά, και προφανώς δεν βγαίνουν κερδισμένοι σε σχέση με το Strawberry.

   > **[ενημ. 2026-08-07] Μετά την ανακοινωμένη balance change το melon παύει να είναι στρατηγική
   > επιλογή.** **Κανένα** από τα 8 shops δεν αγοράζει melon (ούτε fertilizer) — είναι **100%
   > εξαρτημένο από το town center**, το μόνο προϊόν με αυτή την ιδιότητα. Με το TC να πέφτει από
   > 140 σε **30 μονάδες/σεζόν** (−79%) και cliff στις 158 μονάδες, η καμπύλη δεν προλαβαίνει ποτέ
   > να ανακάμψει. **Στρατηγικά αυτό είναι ευκαιρία, όχι απειλή:** εμείς φυτεύουμε **0 melon**, ενώ
   > το meta φυτεύει **11,6 melon seeds/παίκτη** και πουλά batch 12,8 τη μέρα 10
   > ([meta0807](meta/ladder_snapshots.md#meta0807)) — δηλαδή η αλλαγή χτυπά τους αντιπάλους μας
   > πολύ πιο δυνατά από εμάς. Το «οπορτουνιστικά, λίγα tiles» της αρχικής σύστασης γίνεται
   > «**καθόλου**», εκτός αν μετρηθεί το αντίθετο.
6. **Timing πωλήσεων ↔ town ramp**: η ζήτηση ×2 από μέρα 10, ×4 από μέρα 20 και τα shops προστίθενται κάθε 3 μέρες — **ίδια αγαθά αξίζουν περισσότερα αργότερα**, εντός των ορίων του shed cap 100. Trickle selling πάντα (orders λύνονται unit-by-unit)· ποτέ dump στο floor.
   ...
   > ⚠️ **[ενημ. 2026-08-07] Το «αξίζουν περισσότερα αργότερα» έχει ημερομηνία λήξης.** Οι οργανωτές
   > **ανακοίνωσαν** κατάργηση ολόκληρου του ramp (flat ×1) **και** `townCenterSellInterval` 12 → 24
   > (1 tick/μέρα αντί 2). Σεζόν ανά προϊόν: `10×2 + 10×4 + 10×8 = 140` → `30×1 = 30` μονάδες,
   > δηλαδή **−79%**, με **−87,5%** στις μέρες 20-29 — ακριβώς τη φάση ρευστοποίησης. Επαληθεύσαμε
   > τοπικά ότι το **1.32.5 δεν τις περιέχει ακόμα** (diff sdist vs `engine_reference/`: μόνο το
   > shed/LOCKED fix· `.json` diff κενό) — άρα **ανακοινωμένες, αδημοσίευτες**. Δεν αλλάζουμε
   > tuning σήμερα· αλλά κάθε στρατηγική που *στηρίζεται* στο late ramp είναι εκ των προτέρων
   > εύθραυστη. Το trickle selling παραμένει σωστό και στα δύο καθεστώτα.
   > Πλήρης ανάλυση: [meta/competition_updates.md 2026-08-07](meta/competition_updates.md).
...
8. **Παρατήρηση `unlocked_shops`**: ποια shops άνοιξαν είναι τυχαίο ανά episode — το crop mix mid-season πρέπει να προσαρμόζεται στη ζήτηση που όντως ξεκλείδωσε (π.χ. Pet Café → 12 carrots/μέρα, Yarn Store → 2× wool).
   ...
   > **[ενημ. 2026-08-07] Αναβάθμιση από «παρατήρηση» σε σχεδιαστική απαίτηση.** Δύο λόγοι:
   >
   > **(i) Σήμερα δεν το κάνουμε καθόλου.** Το [agent/state.py](../agent/state.py):14 **αφαίρεσε**
   > το `unlocked_shops` από το snapshot ως «zero real readers» (review L9) — σωστό τότε, τρύπα τώρα.
   >
   > **(ii) Η ανακοινωμένη αλλαγή «shops sampled WITH replacement» το κάνει κρίσιμο.** Με 8
   > κληρώσεις από 8 τύπους (cap `MAX_SHOP_INSTANCES = 8`), οι αναμενόμενοι **διακριτοί** τύποι
   > πέφτουν σε `8·(1−(7/8)⁸) = 5,25`. Δηλαδή ένα προϊόν μπορεί να μείνει **χωρίς κανέναν αγοραστή**:
   >
   > | Προϊόν | Shops που το αγοράζουν | P(κανένα) | Cliff (μον. ως $1) |
   > |---|---|---|---|
   > | **WOOL** | YARN_STORE μόνο (single ⇒ 12/μέρα) | **34,4%** | **59** |
   > | CARROT / EGG / TOMATO | 2/8 | 10,0% | 842 / >2.000 / — |
   > | MILK | 3/8 | 2,3% | **76** |
   > | STRAWBERRY | 4/8 | 0,39% | 62 |
   > | WHEAT | 5/8 | **0,04%** | **>2.000** |
   > | MELON | **κανένα, ποτέ** | 100% | 158 |
   >
   > Η αλλαγή δεν είναι tail risk: **1 στα 3 επεισόδια** χάνει τον μοναδικό αγοραστή του wool.
   >
   > **[ενημ. 2026-08-07 — ανεξάρτητη επιβεβαίωση + το κρίσιμο σκέλος χρονισμού.]** Το community
   > notebook `notebooks/your-seed-does-not-fix-the-town.ipynb` (**evidence, όχι πηγή κώδικα** —
   > Ανοιχτό #11) υπολογίζει τα ίδια μεγέθη πάνω στο ίδιο PR diff και στο ίδιο 1.32.5 και
   > **συμφωνεί ακριβώς** (34,4% · 5,25 διακριτοί · wool cliff 59 · shops ανά προϊόν). Προσθέτει
   > τρία που δεν είχαμε: **P(και οι 8 τύποι παρόντες) = 8!/8⁸ = 0,24% ≈ 1 παιχνίδι στα 416**
   > (δηλαδή η σημερινή «όλα ανοιχτά» πόλη, πάνω στην οποία είναι tunαρισμένο κάθε config,
   > σχεδόν εξαφανίζεται)· **late-season wool demand 20 → 13 μον./μέρα με yarn store, → 1 χωρίς
   > (−95%)**· και τιμολόγηση κοπαδιού μέσω μοντέλου αγοράς επαληθευμένου έναντι του engine
   > (both-players-pass, ταιριάζει και στα 9 προϊόντα): 6 sheep **$39,1k με yarn store έναντι
   > $11,1k χωρίς**, 6 cow **$42,4k median**, και σε 500 κληρωμένα towns το **cow** κοπάδι είναι
   > το κερδοφόρο στο **70,2%** έναντι **28,2%** για sheep. (Το μοντέλο αγνοεί τον αντίπαλο που
   > πουλά στην ίδια αγορά ⇒ **άνω φράγμα**· διαβάζουμε τους *λόγους μεταξύ towns*, όχι τα ποσά.)
   >
   > **Το σκέλος που αλλάζει σχεδιασμό:** shops ξεκλειδώνουν από τη **μέρα 3**, pastures και ζώα
   > αγοράζονται στο **άνοιγμα** ⇒ **δεσμεύεσαι στο κοπάδι πριν ξέρεις την πόλη**. Η σύνθεση
   > κοπαδιού είναι απόφαση **υπό αβεβαιότητα**, όχι βελτιστοποίηση, και **καμία** sell-side
   > προσαρμογή δεν την ξεκάνει εκ των υστέρων. Δύο συνέπειες:
   > (i) ~~το shop-adaptive sell layer γίνεται ο μοναδικός διαθέσιμος μηχανισμός αντιστάθμισης~~
   > **⛔ ΔΙΑΨΕΥΣΤΗΚΕ ΜΕ ΜΕΤΡΗΣΗ (2026-08-07)** — υλοποιήθηκε (`current_phase.md` §v1g.2 γ) και
   > **δεν μετριάζει καν τη ζημιά**: μια πόλη χωρίς YARN_STORE κοστίζει **$12.077/ep** από μόνη
   > της, και το sell layer ανακτά **$0** από αυτά (−$1.909/ep όταν ενεργοποιείται εκεί, +$0
   > στην καλύτερη δυνατή ρύθμιση, 0/8 episode wins σε κάθε πόλη). Αιτία: ο agent είναι
   > **production-constrained, ποτέ glut-constrained** — ο ρυθμός πώλησής μας είναι κάτω από την
   > απορρόφηση των NPC ακόμα και με μόνη τη ζήτηση του town centre, οπότε το inventory μένει
   > κάτω από το I0 και εισπράττουμε πάντα την πλευρά της **σπάνης**· η πλευρά του glut που ένα
   > floor προστατεύει δεν φτάνεται ποτέ. Το κόστος της πόλης είναι **παραγωγικό**, όχι θέμα
   > χρονισμού πώλησης, άρα **δεν υπάρχει sell-side αντιστάθμιση**·
   > (ii) **γι' αυτόν ακριβώς τον λόγο** το (ii) έμεινε το **μόνο** ανοιχτό αντίμετρο: πριν
   > πολλαπλασιαστούν τα pastures (SW quadrant), η **σύνθεση** απαιτεί robustness screen κάτω από
   > πολλαπλά κληρωμένα towns — `current_phase.md` §v1h′.
   >
   > **⛔ ΕΤΡΕΞΕ ΚΑΙ ΑΥΤΟ, ΚΑΙ ΤΟ 70,2% ΔΕΝ ΜΕΤΑΦΡΑΖΕΤΑΙ (2026-08-08).** Το screen έγινε ακριβώς
   > όπως ζητήθηκε (`pinned_shops(basket_for(seed))`, DEV_SEEDS, both seats, vs
   > `checkpoints/v1g_1`): **και οι δύο** cow-βαρύτερες συνθέσεις REGRESSED — `{8C,2S}`
   > −$5.093/ep (1/48 seed wins) και `{10C,0S}` −$6.845/ep (6/48, **και** hard-gate αποτυχία).
   > Το `{6C,4S}` του v1g μένει. **Και ο μηχανισμός είναι το πραγματικό εύρημα:** η ζημιά **δεν**
   > συγκεντρώνεται στις πόλεις χωρίς YARN_STORE — εκεί το `{8C,2S}` χάνει **λιγότερο**
   > (−$3.775) απ' ό,τι στις πόλεις **με** yarn store (−$6.035). Άρα ο περιορισμός δεν είναι «ο
   > σπάνιος αγοραστής» αλλά **ο κορεσμός του MILK σε mirror**, όπου πουλάνε **και οι δύο**
   > παίκτες στην ίδια αγορά. Αυτή είναι η πρώτη φορά που μετριέται το όριο του
   > **single-player** μοντέλου του notebook: η επιφύλαξη «άνω φράγμα» του συγγραφέα δεν αφορά
   > μόνο τα **ποσά**, αφορά και την **κατάταξη** — οι *λόγοι μεταξύ towns* που θεωρούσαμε
   > ασφαλείς αντιστρέφονται μόλις μπει αντίπαλος. **Η διαφοροποίηση WOOL/MILK αξίζει
   > περισσότερο από την επιλογή του «στατιστικά καλύτερου» ζώου.** Γενικός κανόνας που
   > προκύπτει: κάθε single-player μοντέλο αυτού του dataset δίνει **υποθέσεις προς μέτρηση**,
   > ποτέ κατατάξεις προς υιοθέτηση. memory.md 2026-08-08.
   > **Συνέπεια για την ιεράρχηση:** WHEAT ↑↑ (μέγιστη κάλυψη, καμία cliff, ήδη υποχρεωτικό ως
   > ζωοτροφή, και με δωρεάν fertilizer φτάνει το cap 6) · STRAWBERRY ↑ · MILK ↑ · **WOOL ↓** ·
   > CARROT ↓ · MELON ↓↓↓. Το feature αφορά **ΜΟΝΟ την πώληση**, όχι τον planner: τα shops
   > ξεκλειδώνουν κάθε 3 μέρες, ενώ το strawberry θέλει 16 μέρες — όταν μαθαίνεις τη ζήτηση, η
   > απόφαση φύτευσης έχει ήδη ληφθεί. Υλοποίηση/παγίδες: `current_phase.md` §v1g.2.
...
**[ενημ. 2026-08-07 — αναθεώρηση του άξονα (α): η «σύγκλιση προς το modal farm» περιορίζεται σε
δομή, και ρητά ΟΧΙ σε χαρτοφυλάκιο.]** Ο άξονας (α) γράφτηκε με την υπόθεση ότι το modal top farm
είναι έγκυρη εκτίμηση του engine optimum. Ισχύει για **γη, crew και ζώα** — δηλαδή για ό,τι
επιβιώνει ως τη μέρα 30 — αλλά **όχι για τα crops**, γιατί εκεί η μέτρηση είναι artifact
(απόδειξη: [meta0807](meta/ladder_snapshots.md#meta0807), engine `:411-412` και `:742-744`,
σύνοψη στην §1 παραπάνω). Τρεις συγκεκριμένες συνέπειες στο roadmap:

1. **Κανένα portfolio rebalance προς το modal.** Το v1h έχασε το «rebalance χαρτοφυλακίου» από
   το scope του (→ `v1h′`, μόνο SW quadrant). Αντιγραφή ενός σφάλματος μέτρησης είναι χειρότερη
   από καθόλου αντιγραφή, γιατί φέρνει βεβαιότητα χωρίς πληροφορία.
2. **Το crew target κλείνει στα 6.** Το «12 hands» της ελίτ ήταν μέτρηση μίας ημέρας· η επόμενη
   έδωσε **5-6** με ρητή αιτιολόγηση (fib hire-cost ceiling). Το δικό μας v1f είχε ήδη καταλήξει
   στο **6** από ανεξάρτητο μονοπάτι (h10/h12 `REGRESSED` — idle hands σε σταθερό tile ceiling).
   Δύο μηχανισμοί, ένα νούμερο: σταματάμε εκεί.
3. ~~**Το ~13 ζώα παύει να είναι στόχος και γίνεται ερώτημα.**~~ **ΑΠΑΝΤΗΘΗΚΕ 2026-08-07 (v1g):
   η κορυφή είναι στα 10 ζώα, όχι 13-14.** Το ερώτημα τέθηκε ως εξής: με cliffs στα **59 (wool)**
   και **76 (milk)** μονάδες, το marginal ζώο μπορεί να πουλά κοντά στο floor, και ένα `IMPROVED`
   του v1g έναντι του 3-ζώων v1f δεν το απαντά. Απαντήθηκε με **δύο** ανεξάρτητες μετρήσεις
   (πλήρη στοιχεία: memory.md 2026-08-07): (i) realized avg sell price ανά προϊόν —
   **WOOL $234-244, MILK $267-272**, δηλαδή πολύ **πάνω** από base, καμία ένδειξη κορεσμού, γιατί
   η ημερήσια NPC ζήτηση καταναλώνει τις μονάδες ταχύτερα απ' ό,τι τις παράγουμε· (ii) screen
   μεγέθους κοπαδιού σε DEV — 7 ζώα `IMPROVED` +$24,4k, **10 ζώα (6 COW + 4 SHEEP) peak
   +$25,4k**, 11 ζώα χειρότερα, 12-14 ζώα **αποτυγχάνουν hard metric gates** (660-885
   `animals_escaped`). Ο περιοριστικός παράγοντας αποδείχθηκε το **feed logistics**, όχι ο
   κορεσμός της αγοράς. Το «~13 ζώα» της ελίτ **αποσύρεται ως capacity target**.
   ⚠️ Ανοιχτό υπο-ερώτημα (`current_phase.md` §0bis ⚠️ζ): οι *πουλημένες* μονάδες wool είναι
   μόλις **45%** του θεωρητικού μεγίστου του κοπαδιού (milk: 88%) — άγνωστο αν είναι χαμένη
   παραγωγή ή απούλητο inventory. Προϋπόθεση πριν εξεταστεί ποτέ αύξηση SHEEP.

**Το γενικό μάθημα, τρίτη φορά που εμφανίζεται** (μετά το «land day 0» του §3.2bis και το
«carrot 0% win rate» του n=5): *ένα aggregate από replays μετρά πάντα κάτι ελαφρώς διαφορετικό
από αυτό που νομίζεις.* Πριν χτίσουμε πάνω σε οποιοδήποτε meta νούμερο, η ερώτηση δεν είναι «τι
λέει» αλλά **«σε ποια χρονική στιγμή και σε ποια οντότητα μετρήθηκε»**.
...
- **(ε) [νέο 2026-08-07] Ανθεκτικότητα στη σύνθεση της ζήτησης** (§3.2#8): ο agent πρέπει να διαβάζει **ποια** ζήτηση υπάρχει (`unlocked_shops` → `npc_daily_demand` ανά προϊόν) και να ρυθμίζει **την πώληση** ανάλογα. Αιτιολόγηση σε δύο επίπεδα: **σήμερα** η σειρά ξεκλειδώματος είναι τυχαία (ζήτημα χρονισμού)· **μετά την ανακοινωμένη αλλαγή** ένα προϊόν μπορεί να μείνει μόνιμα χωρίς αγοραστή (**34,4%** για το wool). ~~Το feature αποδίδει και στα δύο καθεστώτα~~ — **⛔ ΔΙΑΨΕΥΣΤΗΚΕ ΜΕ ΜΕΤΡΗΣΗ (2026-08-07, `current_phase.md` §v1g.2 γ): δεν αποδίδει σε κανένα.** Το «διάβασε ποια ζήτηση υπάρχει» **έγινε** και μένει (`unlocked_shops` → `agent/demand.py`, engine-pinned)· το «ρύθμισε την πώληση ανάλογα» **μετρήθηκε αρνητικά παντού** (−$1.103/ep πλήρης πόλη, −$1.909/ep χωρίς YARN_STORE, −$24.762/ep χωρίς shops, 0/8 wins· +$0 στην καλύτερη ρύθμιση). Ο agent είναι **production-constrained, ποτέ glut-constrained**, άρα ένα demand-sized sell floor δεν έχει τιμή να κερδίσει — μόνο όγκο να χάσει (WOOL: ίδια τιμή/μονάδα $241,46 → $241,37, λιγότερες μονάδες 348 → 328). Ο άξονας **δεν** καταργείται: μετατοπίζεται από την **πώληση** στην **παραγωγή/σύνθεση** (§3.4 άξονας (α), `current_phase.md` §v1h′). Δεύτερο σκέλος του ίδιου άξονα, και το μόνο που **επιβεβαιώθηκε**: **fertilizer έχει μηδενική NPC ζήτηση εξ ορισμού** ⇒ μονότονα φθίνουσα τιμή ⇒ «πούλα νωρίς, πάντα» είναι κανόνας, όχι tuning — και είναι ακριβώς η **αντίθετη** συνταγή από αυτήν που το διαψευσμένο sell layer ζητούσε για ζήτηση κοντά στο μηδέν.
...
> **[νέο 2026-08-07] Το «ίδιο seed» δεν σημαίνει «ίδιος κόσμος»: κατάταξη knob πριν από κάθε A/B.**
> Άμεση συνέπεια της §2 Αμφισημίας #7 (shop unlock και weed spawning μοιράζονται το per-day RNG
> stream, με τη θέση της κλήρωσης να ισούται με τα άδεια tiles **και των δύο** φαρμών). Άρα:
>
> - **Market-only knob** (sell batch, sell floor, order tier/σειρά — οτιδήποτε αγγίζει μόνο τη
>   λίστα market orders): **δεν** μπορεί να μετακινήσει tile ⇒ και τα δύο arms τραβούν **την ίδια
>   πόλη** ⇒ το σταθερό seed είναι γνήσια ελεγχόμενο πείραμα. Μετρημένο side-by-side: ίδια πόλη σε
>   **16/16** seeds, effect **10 coins** επιλύσιμο από ελάχιστα episodes.
> - **Occupancy knob** (labour/crew, φύτευση, συγκομιδή, DIG, BUY_LAND, τοποθέτηση ζώων/δομών,
>   routing που μετακινεί εργασία μέσα στη μέρα): ξαναρίχνει την πόλη σε **κάθε** seed
>   (**0/16** ίδια πόλη στη μέτρηση) και ο paired θόρυβος πάει από μερικά coins σε **>$1.000**.
>   Απαιτεί είτε pinned towns και στα δύο arms είτε ρητά αιτιολογημένο μεγαλύτερο seed budget.
> - **Το pinning μειώνει, δεν εξαλείφει** (~19% του noise sd, ≈1,5× τα episodes): δεν αγγίζει τα
>   weeds (ο player 0 τραβά πριν τον player 1) και δεν πρέπει να εξαφανίσει γνήσιες
>   seed-dependent αλληλεπιδράσεις — αυτές είναι **σήμα ανθεκτικότητας** (άξονας δ), όχι θόρυβος.
> - **Ένα pinned αποτέλεσμα ισχύει μόνο για τις πόλεις που πινάρισες**, άρα το τελικό
>   holdout-confirm τρέχει **και χωρίς pin**.
>
> Εργαλείο και επιχειρησιακός πίνακας απόφασης: `harness/town_pin.py` και
> `current_phase.md` §Πρωτόκολλο («ΥΠΟΧΡΕΩΤΙΚΟ pre-gate ερώτημα»).
...
| 11 | **[1.32.5] Shed ops σε LOCKED tile** | ≤1.32.4: `PICKUP`/`DROP`/`PLACE` έκαναν no-op αν το tile ήταν LOCKED | **≥1.32.5: δουλεύουν** — τα shed ops λύνονται **πριν** τον LOCKED guard. Τρία από τα 4 shed-access tiles ξεκινούν locked, άρα ο shed είναι πλέον προσβάσιμος και από τα 4. **Ο [agent/scheduler.py](../agent/scheduler.py):182 έχει ακόμα hardcoded `access = (4, 4)`** — αληθές στο 1.32.4, ψευδές μετά. ⚠️ **Η διόρθωση δοκιμάστηκε στο v1g.1, μετρήθηκε net-negative και έγινε revert** (η υπερεκτίμηση απόστασης του παλιού `(4,4)` φούσκωνε τεχνητά το `urgency_tier` του WHEAT PICKUP και αυτό ήταν κατά λάθος ωφέλιμο)· απαιτεί πρώτα decoupling routing-distance από urgency-distance μέσα στο `assign()`. Πλήρης ανάλυση: memory.md 2026-08-07 |
...
1. **Κινούμενο engine**: 2 versions σε μία εβδομάδα (locked-tile passability άλλαξε συμπεριφορά spawn/movement). Κάθε νέα έκδοση `kaggle-environments` απαιτεί re-run του regression suite. Mitigation: pinned version (**1.32.4**, εγκατεστημένο σε `.venv/` — δες §2 Αμφισημία #2) + micro-tests όποτε αλλάξει.
   ...
   > **[ενημ. 2026-08-07] Τρίτη έκδοση: 1.32.5** (απόκλιση #11 παραπάνω). Ο κανόνας ανίχνευσης
   > έγινε συγκεκριμένος και **δεν απαιτεί install** — κρίσιμο, γιατί ένα `pip install -U` στη μέση
   > ενός gate ακυρώνει τη σύγκριση: `pip index versions` → `pip download --no-deps --no-binary :all:`
   > σε scratchpad → `diff` του `.py` **και του `.json`**. Το `.json` δεν είναι προαιρετικό: εκεί ζουν
   > `townCenterSellInterval`, `turnsPerDay`, `maxMarketOrdersPerTurn`, `shedCapacity` — **μια balance
   > change μπορεί να είναι αποκλειστικά json και αόρατη στον κώδικα**. Πλήρης διαδικασία:
   > `current_phase.md` ΜΕΡΟΣ Γ #4.
...
9. **[νέο 2026-08-07] Ανακοινωμένη balance change που δεν έχει κυκλοφορήσει.** Οι οργανωτές ανακοίνωσαν town-center demand **−79%** (flat ×1, 1 tick/μέρα) και **shops sampled with replacement** (cap 8 instances). Επαληθεύσαμε ότι το **1.32.5 δεν τις περιέχει** — άρα η ladder τρέχει ακόμα το παλιό μοντέλο. **Το ρίσκο δεν είναι η αλλαγή, είναι ο χρονισμός της:** αν κυκλοφορήσει ενώ έχουμε κατεψυγμένο submission, το rating πέφτει χωρίς προφανή αιτία, και τα 2 active slots μας κρατούν έναν agent tuned για νεκρό μοντέλο ζήτησης. **Mitigation, τρία σκέλη:** (i) ο detector του #1 τρέχει **τακτικά**, όχι μόνο όταν κάτι σπάσει· (ii) ο άξονας (ε) — shop-adaptive πώληση — κάνει τον agent ανθεκτικό **και στα δύο** καθεστώτα χωρίς να στοιχηματίζει σε κανένα· (iii) ο robustness matrix της Φάσης 3 περιλαμβάνει σενάριο **«flat town center»** με monkeypatch στο harness, ώστε να ξέρουμε το κόστος **πριν** το πληρώσουμε ζωντανά. Ό,τι κι αν γίνει, η κατεύθυνση της αλλαγής μας ευνοεί σχετικά: το melon (0 tiles για μας, 11,6 seeds/παίκτη για το meta) είναι ο μεγαλύτερος χαμένος.

10. **[νέο 2026-08-07] Εγκυρότητα μέτρησης: τα fixed-seed A/B μας δεν είναι όλα ελεγχόμενα πειράματα.** Το shop unlock κληρώνεται από το **ίδιο** per-day RNG stream που μόλις κατανάλωσε το weed spawning **και των δύο** παικτών (§2 Αμφισημία #7), οπότε κάθε αλλαγή στην occupancy ξαναρίχνει την πόλη. *Γιατί είναι ρίσκο και όχι απλή λεπτομέρεια:* ολόκληρο το gate πρωτόκολλο της Φάσης 2 στηρίζεται σε paired seeds, και μια ολόκληρη κατηγορία των increments μας (crew, γη, ζώα, routing) ανήκει στην «μολυσμένη» πλευρά. Το ρίσκο **δεν** είναι λάθος πρόσημο — ο town θόρυβος είναι συμμετρικός και δεν παράγει συστηματική φορά — αλλά **λάθος μέγεθος** και **ψευδώς σφιχτά CI**, δηλαδή κατατάξεις μεταξύ κοντινών variants (screen «top-3», BBO «κράτα το max») που είναι θόρυβος. **Mitigation:** το υποχρεωτικό pre-gate ερώτημα του §6 + `harness/town_pin.py` για occupancy gates· ρητή σημείωση σε κάθε παλιό αποτέλεσμα του οποίου η *κατάταξη* (όχι η κατεύθυνση) στηριζόταν σε occupancy knob — απογραφή στο `current_phase.md` §Β.1.
