...
3. **Το ~13 ζώα παύει να είναι στόχος και γίνεται ερώτημα.** Με cliffs στα **59 (wool)** και
   **76 (milk)** μονάδες, και δική μας παραγωγή ~160/~264 μονάδες αντίστοιχα (και **διπλάσια σε
   mirror**), το marginal ζώο μπορεί να πουλά κοντά στο floor. Ένα `IMPROVED` του v1g έναντι του
   3-ζώων v1f **δεν** το απαντά. Απαντιέται μόνο με realized avg sell price ανά προϊόν
   (`current_phase.md` §Β.0.1) και, αν χρειαστεί, screen `{8,5}` vs `{6,3}` vs `{8,3}`.
...
- **(ε) [νέο 2026-08-07] Ανθεκτικότητα στη σύνθεση της ζήτησης** (§3.2#8): ο agent πρέπει να διαβάζει **ποια** ζήτηση υπάρχει (`unlocked_shops` → `npc_daily_demand` ανά προϊόν) και να ρυθμίζει **την πώληση** ανάλογα. Αιτιολόγηση σε δύο επίπεδα: **σήμερα** η σειρά ξεκλειδώματος είναι τυχαία (ζήτημα χρονισμού)· **μετά την ανακοινωμένη αλλαγή** ένα προϊόν μπορεί να μείνει μόνιμα χωρίς αγοραστή (**34,4%** για το wool). Το feature αποδίδει και στα δύο καθεστώτα, άρα **δεν είναι στοίχημα πάνω στην ανακοίνωση** — γι' αυτό μπαίνει τώρα (`v1g.2`) και όχι «αν/όταν» κυκλοφορήσει. Δεύτερο σκέλος του ίδιου άξονα: **fertilizer έχει μηδενική NPC ζήτηση εξ ορισμού** ⇒ μονότονα φθίνουσα τιμή ⇒ «πούλα νωρίς, πάντα» είναι κανόνας, όχι tuning.
