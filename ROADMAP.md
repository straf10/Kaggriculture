...
of the current top-4 (15 each, per-team median **range** across the four). ⚠️ **That script
(`analysis/b1_top4_profile_2026_08_21.py`) and its dependency were pruned 2026-08-24** (eafcd43,
§11's "§5.1 top-N profile is stale" row) — on top of the untracked collector chain that fed the
*original* fit, closed earlier (§11's last row). The re-fit below is now stale and not directly
reproducible. Its replacement, `analysis/b1_top_profile.py`, reads off `analysis/archive_sync.py
top K N`'s selection instead (submission-scoped API only, no collector chain); re-running it is
the way to refresh this table. **The shape holds; the levels moved where marked.**
...
| 37 | **S15 Phase 1 — MELON wave-2 re-phasing STOPS at gate 2** | KILL by the plan's own pre-registered escalation gate, before any `agent/` code. **Gate 1 (confirm-set replication) PASSED**: `55586926`+`55675634`, 539 episodes (ROADMAP §8), untouched by S14 — at the same d16 harvest day, `d_our_melon_median` reads **+$4.558** (1,16× the screen figure +$3.918, ≥0,60× required) and the loss-flip share reads **138/250 = 0,5520** (within 0,08 of the screen's 32/65 = 0,4923). **Gate 2 (hand-routing feasibility) STOPS it.** Replaying the recorded action stream itself (not a model) over d6h17-d7h23 (31 turns, indices `day*24+hour` — verified against real replays, `tests/test_s15_gate2_hand_routing.py::test_replay_index_matches_requested_day_hour`) — the window `g3_feasibility` names as earliest land+cash-feasible — across all 698 ladder episodes of all three live submissions (9 distinct signatures, dominant 452/698 — deterministic on `55586926`/`55726984`, near-identical variants on `55675634`'s tile-recovery arm): idle hand-turn capacity tops out at **25**, short of the **42-turn floor** for 14 tiles (1 move + 1 PLANT + 1 same-day WATER each — the day-boundary weed-death rule kills any tile left unwatered on its own planting day, `engine_reference/kaggriculture.py:769-784`); free NE tiles once the window's last action lands bottom out at **6** (median 8, max 9), short of the **14** needed. Mechanism: the tape already spends the newly-unlocked NE land on its own recorded **12 STRAWBERRY + 4 WHEAT + up to 4 PASTURE** plantings inside the identical window, and idle hand-turns only appear once that land is already spoken for (day-7 evening, by which point only 6-9 tiles remain). The d7/d17 fallback does not rescue this: the recorded STRAWBERRY block plants straight through d7h22, so the conflict holds inside the fallback window too. Confirms §2's own red flag (point 5) rather than resolving it — gates 3 (opponent-reaction sensitivity) and 4 (threshold) were not run, per the plan's rule that any one gate failing is a STOP, not a re-scope. No `agent/` code, no upload. `data/derived/s15_gate1_confirm_replication.json`, `data/derived/s15_gate2_hand_routing.json`, `tests/test_s15_gate2_hand_routing.py`, [docs/plans/s15_melon_rephasing.md](docs/plans/s15_melon_rephasing.md) |
...
| ~~**The §5.1 top-N profile is stale**~~ | ⚠️ **RE-OPENED 2026-08-29** — the 2026-08-21 re-fit's script (`analysis/b1_top4_profile_2026_08_21.py`) and its dependency were pruned 2026-08-24 (eafcd43, "prune 53 superseded one-off analysis scripts") and are gone from the working tree; the fit in §5.1's table is now stale and not directly reproducible. Replacement built 2026-08-29: [analysis/b1_top_profile.py](analysis/b1_top_profile.py), which reads `analysis/archive_sync.py top K N`'s selection (submission-scoped `kaggle` CLI only, no collector chain) — run `archive_sync.py board && archive_sync.py top 5 10 && b1_top_profile.py --selection ...` to refresh §5.1 | Refit against the 60 fresh top-4 traces pulled by the §7.1 selection pass. Old profile largely holds: quadrants 3 (SE never), first extra day 5-7, second 8-10, d29 end $82,5-$98,4k (was $82,7k median). What moved: **COW peaks 7-8 (was 8-9), SHEEP more variable (2-7 vs 4-5), MELON tile-days 110-180 (was 180), WHEAT first sell d0-5 (was d5/6)**. The old b1_top5_profile.py depended on the *earlier* retired collector chain (§11 last row) and could not re-run without it; the replacement reads kaggle-CLI-fetched replays directly, selected via the archive_sync `top` command (§9) |
