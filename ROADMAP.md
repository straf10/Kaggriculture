...
| 🟢 Our best public score | **1.906,5** — `55586926`, the **ReCurSiON majority-vote reconstruction**, read 2026-08-18 **10:32 UTC on 88 episodes** (peak 1.915,8 at 85). **Rank ~940 / 5.127** (from 1.263 / 4.947 on 08-17 — **+325 places**). Past the evicted Valmorlee tape's converged 1.599,1, our previous best 1.617,6, and ~2,9× every hand-built agent this repo produced (652,5, v1h) | `kaggle competitions submissions -v` · `leaderboard -d` |
...
> Phase 0 assumed.** ReCurSiON's strawberry sell-timing is a **fixed global calendar** (hold strawberry,
> release the shed into the **hour-0 town-center pulse**, `townCenterSellInterval=24`), **invariant
> across all 50 towns** (46/50 traces sell an identical **290 units**; at step 336 all 50 sell exactly 6
> while price spans **$151–$230** and the strawberry-shop draw spans **0–4**; corr(units, own shed)
> **+0,92** vs corr(units, shop identity) **+0,02**). It conditions on **neither shop identity nor own
> varying state** — and **the majority vote already reproduces it** (reconstruction sells 290). So there
> is **no erased conditioning to restore**: arm A is a no-op, arm C moot, arm B the forbidden invention
> (surface area now measured **≈ $0**, zero-sum, behind the T2 shed wall). **Live:** our reconstruction
> executes the calendar (**81,8%** hour-0) but gets parity because our live opponents also concentrate
> (**60,8%** hour-0) — the $117→$90 collapse is the **zero-sum contest**, not lost sell-timing. Every
> measured GO number reproduces (ratio 1,32; volume 286=286); the *interpretation* ("the vote erased
> town-conditioning") is overturned. **STOP before building arms; the strawberry overlay closes (§3.3).**
> Report: `baselines/2026-08-18/s6_step2b_phase05_report.md`; script `analysis/s6_step2b_phase05.py`;
> guards `tests/test_s6_step2b_phase05.py`. No `agent/` change, no upload (R27). The open question the
> numbers now point to is §4.4#1's clock and the endgame (43 days to 2026-09-30), not another overlay.

*The Phase-0 GO below is kept verbatim for its measurements, which all reproduce; only its causal
interpretation ("the vote erased town-conditioning") is superseded by Phase 0.5 above.*

#### S6 step 2b — Phase-0 GO (2026-08-18, superseded on interpretation): the gap is the erased sell-timing; GATE CLEARS → build the overlay
...
##### Second read of the 2b Phase-0 report (2026-08-18, evaluation) — GO confirmed, two claims deflated

**The GO is right and the central measurement is the best-controlled thing in this repo.** The
pre-registered refutation fired on its own terms; the strawberry ratio was computed by the *same*
instrument as the donor's 1,339 (`_realised_premium`, focal/opp); the field calibration (ReCurSiON 1,339
an outlier, next best boatlee 1,117, rest ~1,00) rules out a town or opponent artefact because the town
is common-mode to both seats and no other team shows it; production is proven to have transferred whole
(**volume 286 = 286**), which kills component (b) outright; and the §4.1b reconciliation is exactly right
— §4.1b's control ran **modal route vs modal route** and said so, which is structurally blind to an edge
both seats share. **Two instruments agreeing on one number, one of them our own live ladder episodes, is
the strongest evidence this repo has produced. Build it.**

**Two claims in the report are not supported by their stated evidence, and neither changes the GO:**

1. ⚠️ **"Worth many hundreds of rating points" is asserted, not derived — the dollars are measured, the
   conversion is not.** The report correctly rejects §3.4's $253/ep marginal rate (its amendment does say
   a premium-price edge converts better), but then substitutes a qualitative argument. What is actually
   measured is **+$7.833/ep against a +$1.076 median margin in our contested band** — which makes "this
   flips many close episodes" very likely and "many hundreds of points" a guess. Quote the dollars; let
   the ladder price the points.
2. 🔴 **The opponent-rating band table cannot carry the two conclusions resting on it (→ R36).** Its win
   rates are **not compatible with a converged 1.906,5**: at that rating the Elo-expected score against a
   ~1.750 opponent is ~0,71 (observed **0,62** — we *under*perform) and against a ~2.250 opponent ~0,12
   (observed **0,65**, n=20 — we overperform by 5×). A converged rating cannot sit at 1.906 while
   genuinely winning 65% of 20 episodes against 2000-2500. The likely cause is the axis itself: opponent
   rating looks to have been joined **from today's leaderboard by team name**, i.e. the team's *current
   best* rating, not the rating of the *submission* we actually played at the time we played it. So
   **"the 50% crossing is at ~2500"** and **"the lever opens the 2500-3000 band"** are unsupported — the
   latter rests on **n=3**. What survives untouched is the simpler and sufficient argument: a converged
   1.906,5 against a donor at 2.985,6 **is** a 1.073-point strength gap, whatever the pairing looked like.
3. ⚠️ **And the skim is zero-sum, which bounds the lever exactly where we need it.** ReCurSiON reached
   $117,4/u by taking the high-price early demand and leaving its opponent the $89,5 residual. Our
   opponents in the 1500-2500 band mostly *do not* condition, so the lever should be nearly fully
   available there. Against 2500+ opponents — who do — **both** sides skim and neither gets 1,31×. So the
   $7.833/ep is a ceiling *against the pool we currently meet*, and its value in the band that would take
   us past 2800 is **unmeasured**. Measure it in the gate against the strongest available opponents
   rather than assuming it.

**None of this argues for delay.** A +$7.833/ep lever on a $1.076 margin is the largest measured, cleanly
controlled increment in this repo's history, and it is the same mechanism that made the donor #5.
