...
| 33 | **S13 Phase 1 — the seat asymmetry is noise** | KILL by measurement, pre-registered gate. `55726984`'s 97-episode screen (seat 0 30W-16L WR 0,652, seat 1 24W-27L WR 0,471, gap +0,182) does **not** replicate on the independent 412-episode confirm set: `55586926` (293 eps) reads gap **−0,003** (essentially zero), `55675634` (119 eps) reads **+0,095** — **signs disagree across the three submissions**, which alone fails the gate. Headline CMH stratified by rating zone (379/509 episodes matched, 74,5%): **χ²=1,62, p=0,204** (uncorrected 0,168) — nowhere near the 0,01 bar. Both controls came back clean rather than explaining anything: no opponent-strength confound (Mann-Whitney U on matched opponent scores, p=0,264), no town/shop-draw confound (chi² over the 8 shop types by seat, p=0,830), and a pooled logistic regression of outcome on (seat, episode position, submission) found the seat term does not survive — LR p=0,255, Wald p=0,512. **The 18-point gap was the placement-burst screen finding noise in the smallest, newest submission** (§2 rule 2 exists for exactly this). §11 item closed. `analysis/s13_seat_asymmetry.py`, `data/derived/s13_seat_phase1.json`, `tests/test_s13_seat_asymmetry.py` |
...
|---|---|---|
| ~~**Seat asymmetry — 18 points of win rate, unexplained**~~ | ⛔ **CLOSED 2026-08-27 — dead, did not survive the confirm set** (§6 row 33, `docs/plans/s13_seat_asymmetry.md` Phase 1) | The 97-episode `55726984` screen (seat 0 WR 0,652 vs seat 1 WR 0,471) was noise: the independent 412-episode confirm set disagrees in **sign** (55586926 gap −0,003, 55675634 gap +0,095) and the headline CMH stratified by rating zone reads **p=0,204**, far short of the pre-registered 0,01 bar. No confound explained it either (opponent strength p=0,264, town/shop draw p=0,830) — there was simply nothing there. No Phase 2, no `agent/` change, no slot spent |
